...
public class LeavewordController extends BaseController {
	...
	public void userAdd(Leaveword leaveword, 
			Model model, HttpServletRequest request,HttpSession session,HttpServletResponse response) throws Exception {
		String message = "";
		boolean success = false;
	...
	public void list(String title,@ModelAttribute("userObj") UserInfo userObj,String leaveTime,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (title == null) title = "";
		if (leaveTime == null) leaveTime = "";
		if(rows != 0)leavewordService.setRows(rows);
		List<Leaveword> leavewordList = leavewordService.queryLeaveword(title, userObj, leaveTime, page);
	    /*计算总的页数和总的记录数*/
	    leavewordService.queryTotalPageAndRecordNumber(title, userObj, leaveTime);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String title,@ModelAttribute("userObj") UserInfo userObj,String leaveTime,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (title == null) title = "";
		if (leaveTime == null) leaveTime = "";
		List<Leaveword> leavewordList = leavewordService.queryLeaveword(title, userObj, leaveTime, currentPage);
	    /*计算总的页数和总的记录数*/
	    leavewordService.queryTotalPageAndRecordNumber(title, userObj, leaveTime);
	    /*获取到总的页码数目*/
	    int totalPage = leavewordService.getTotalPage();
	...
	public void OutToExcel(String title,@ModelAttribute("userObj") UserInfo userObj,String leaveTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(title == null) title = "";
        if(leaveTime == null) leaveTime = "";
        List<Leaveword> leavewordList = leavewordService.queryLeaveword(title,userObj,leaveTime);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Leaveword信息记录"; 
        String[] headers = { "留言标题","留言人","留言时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<leavewordList.size();i++) {
        	Leaveword leaveword = leavewordList.get(i); 
        	dataset.add(new String[]{leaveword.getTitle(),leaveword.getUserObj().getName(),leaveword.getLeaveTime()});
        }
        /*
